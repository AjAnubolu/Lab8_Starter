...
// The same recipe URLs requested by main.js. They are pre-cached on install so
// the recipe data is available even on the very first offline visit.
const RECIPE_URLS = [
...
self.addEventListener('install', function (event) {
  event.waitUntil(
    caches.open(CACHE_NAME).then(function (cache) {
      // B6. Add all of the URLs from RECIPE_URLS so that they are added to the
      //     cache when the ServiceWorker is installed.
...
self.addEventListener('fetch', function (event) {
  ...
  event.respondWith(
    caches.open(CACHE_NAME).then(function (cache) {
      ...
      //     Otherwise fetch the resource, add it to the cache, and return the
      //     network response.
